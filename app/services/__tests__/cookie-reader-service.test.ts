...
// ADR-005 Phase 2: CookieReader.layerTest バリアントの単体テスト。
...
describe("CookieReader", () => {
  describe("Custom", () => {
    it.effect("指定された session token をそのまま返す", () =>
      ...
      }).pipe(Effect.provide(CookieReader.layerTest("test-token"))),
    );
    ...
    it.effect("undefined を渡すと undefined を返す (未認証分岐の網羅用)", () =>
      ...
      }).pipe(Effect.provide(CookieReader.layerTest(undefined))),
    );
  });
});
